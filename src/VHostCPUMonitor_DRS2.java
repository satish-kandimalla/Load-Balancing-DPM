					...
					for (String metricNam : ConstantUtil.METRIC_LIST) {
						...
						for (String p : metricProps.keySet()) {
							if (ConstantUtil.PROJECT_PARAMETER_LIST_DRS2
									.contains(p)) {
								if (vmStat == -1) {
									vmStat = Integer.parseInt(metricProps
											.get(p));
									minUsageVM = vmObj;
								}

								if (vmStat > Integer.parseInt(metricProps
										.get(p))) {
									vmStat = Integer.parseInt(metricProps
											.get(p));
									minUsageVM = vmObj;
									System.out.println("Vm name is--->"
											+ vmObj.getName());
									selectedVM.put(vmObj.getName(), Integer
											.parseInt(metricProps.get(p)));
								}
							}
						}// end of For metric
					}
...
//		
		System.out.println("Added new host with ip: 130.65.132.133");
